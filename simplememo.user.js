...
(async function() {
 ...
 const USER_NOTE_FOLDER_ID = 'folder_user_note';
 ...
 function getChatId() { const m = location.pathname.match(/\/stories\/[^/]+\/episodes\/([^/?#]+)/) || location.pathname.match(/\/episodes\/([a-f0-9]+)/); return m ? m[1] : null; }
 function getToken() { const m = document.cookie.match(/(?:^|;\s*)access_token=([^;]+)/); return m ? m[1] : null; }
 function getWrtnId() { const m = document.cookie.match(/(?:^|;\s*)__w_id=([^;]+)/); return m ? m[1] : null; }

 function buildHeaders() {
  const token = getToken(), wrtnId = getWrtnId();
 ...
 async function apiLongTermSave(title, pureText) {
  const chatId = getChatId();
  if (!chatId) { alert('채팅 ID를 찾을 수 없습니다.'); return null; }
  return fetch(`${API_BASE}/v3/chats/${chatId}/summaries`, {
   method: 'POST',
   headers: buildHeaders(),
   body: JSON.stringify({ type: 'shortTerm', title: title, summary: pureText })
  }).then(r => r.ok ? true : false).catch(() => false);
 }

 async function apiUserNoteSave(content, isExtend) {
  const chatId = getChatId();
  if (!chatId) { alert('채팅 ID를 찾을 수 없습니다.'); return null; }
  return fetch(`${API_BASE}/v3/chats/${chatId}`, {
   method: 'PATCH',
   headers: buildHeaders(),
   credentials: 'include',
   body: JSON.stringify({ userNote: { content, isExtend } })
  }).then(r => r.ok ? true : false).catch(() => false);
 }
 ...
 if (rawDataV2) {
  ...
  appData.notes.forEach(n => { if (n.isTrashed === undefined) n.isTrashed = false; if (n.isExtend === undefined) n.isExtend = false; });
 ...
 if (!appData.folders.find(f => f.id === USER_NOTE_FOLDER_ID)) {
  appData.folders.splice(1, 0, { id: USER_NOTE_FOLDER_ID, name: "유저 노트", isOpen: true, parentId: null, isSystem: true });
  saveData();
 }
 ...
 function initNotepad() {
  ...
  newNoteBtn.onclick = () => {
   ...
   const newNote = { id: 'note_' + Date.now(), title: "", content: "", folderId: currentFolder, updatedAt: Date.now(), isPinned: false, isTrashed: false, isExtend: false };
  ...
  const renderBody = () => {
   ...
   if (searchQuery.trim() !== "") {
    ...
    const renderSingleFolder = (folder, container, depth) => {
     ...
     const isSystemFolder = folder.id === SYSTEM_FOLDER_ID || folder.id === USER_NOTE_FOLDER_ID;

     fHeader.style.cssText = `display:flex; justify-content:space-between; align-items:center; padding: 8px 14px 8px ${14 + (depth * 14)}px; cursor:pointer; background:transparent; flex-shrink:0; margin: 0 4px; border-radius: 6px; user-select:none;`;
     fHeader.className = 'sn-hoverable sn-folder-header';
     fHeader.dataset.folderId = folder.id;

     const fTitleWrap = document.createElement('div');
     fTitleWrap.style.cssText = "display:flex; align-items:center; overflow:hidden; white-space:nowrap; flex:1; gap:6px; pointer-events:none;";
     ...
     fHeader.addEventListener('touchstart', (e) => {
        if(isSelectMode || isSystemFolder) return;
        if(e.target.closest('.sn-icon-btn')) return;

        longPressTimer = setTimeout(() => {
            draggedFolderId = folder.id;
            fHeader.classList.add('sn-folder-dragging');
        }, 400);
     }, {passive: true});

     fHeader.addEventListener('touchmove', (e) => {
        if (!draggedFolderId) {
        ...
        const targetHeader = targetEl?.closest('.sn-folder-header');

        if (targetHeader && targetHeader.dataset.folderId !== draggedFolderId) {
            const targetId = targetHeader.dataset.folderId;
            const fromIdx = appData.folders.findIndex(f => f.id === draggedFolderId);
            const toIdx = appData.folders.findIndex(f => f.id === targetId);
            if (fromIdx > -1 && toIdx > -1) {
                const item = appData.folders.splice(fromIdx, 1)[0];
     ...
     fHeader.addEventListener('touchend', (e) => {
        clearTimeout(longPressTimer);
        if (draggedFolderId) {
            draggedFolderId = null;
            fHeader.classList.remove('sn-folder-dragging');
            e.preventDefault();
            saveData(); renderBody();
        }
     });
     ...
     if (folder.isOpen) {
      const childFolders = appData.folders.filter(f => f.parentId === folder.id);
    ...
    const unFolder = appData.folders.find(f => f.id === USER_NOTE_FOLDER_ID);
    ...
    if (unFolder) {
     unDescendantIds = getDescendantFolderIds(unFolder.id);
     renderSingleFolder(unFolder, listWrapper, 0);
    }

    if (ltFolder || unFolder) {
     listWrapper.appendChild(createDivider());
    }

    const rootNotes = sortNotes(validNotes.filter(n => !n.folderId));
    const allMemCount = validNotes.filter(n => !ltDescendantIds.includes(n.folderId) && !unDescendantIds.includes(n.folderId)).length;
    ...
    const rootFolders = appData.folders.filter(f => f.parentId === null && f.id !== SYSTEM_FOLDER_ID && f.id !== USER_NOTE_FOLDER_ID);
   ...
   if (activeNote) {
    ...
    const isUserNoteMode = (activeNote.folderId === USER_NOTE_FOLDER_ID);
    const isSystemMode = isLongTermMode || isUserNoteMode;
    ...
    photoBtn.style.cssText = `margin-left:auto; display:${(isTrashed || isSystemMode) ? 'none' : 'flex'}; position:relative; z-index:2;`;
    ...
    editorDiv.setAttribute('placeholder', isUserNoteMode ? '잊으면 안되는 중요한 내용, 추가하고 싶은 설정 등 (유저노트)' : '내용을 입력하세요...');
    editorDiv.style.cssText = `flex:1; background:transparent; border:none; color:var(--sn-textTitle); font-size:15px; outline:none; overflow-y:auto; line-height:1.7; padding:4px 0 24px; width:100%; -webkit-overflow-scrolling:touch; opacity: ${isTrashed ? '0.6' : '1'}; letter-spacing:-0.2px;`;
    ...
    const triggerAutoSave = () => {
      const statusEl = footer.querySelector('.sn-save-status');
      if(statusEl) { statusEl.innerText = "저장 중..."; statusEl.style.color = 'var(--sn-subText)'; }
    ...
    if (isTrashed) {
     ...
     const leftFooter = document.createElement('div'); leftFooter.style.cssText = "display:flex; align-items:center; gap: 12px;";
     ...
     const updateCharCount = () => {
      ...
      if (isLongTermMode) {
          ...
          isOverLimit = plainTextLen > maxLen;
          const extendStr = activeNote.isExtend ? " (자동 확장)" : " (일반)";
          limitText = `${plainTextLen}/${maxLen}자${extendStr}`;
      } else {
          limitText = `${plainTextLen}자`;
      }

      const dateStr = new Date(activeNote.updatedAt).toLocaleDateString([], { month: 'short', day: 'numeric', hour: '2-digit', minute:'2-digit' });
      charCount.innerText = `${dateStr} · ${limitText}`;

      if (isOverLimit) {
          charCount.style.color = 'var(--sn-danger)';
          charCount.style.fontWeight = 'bold';
      } else {
          charCount.style.color = 'var(--sn-textDesc)';
     ...
     if (isSystemMode) {
      ...
      const modeName = isLongTermMode ? "기억" : "유저 노트";
      apiBtn.innerText = `${modeName} 업데이트`;
      apiBtn.style.cssText = `background: var(--sn-accentHover); color: var(--sn-accent); border: none; border-radius: 8px; padding: 8px 12px; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.2s; white-space:nowrap;`;

      apiBtn.onclick = async () => {
       const pureText = getPlainText();
       apiBtn.innerText = "업데이트 중..."; apiBtn.style.opacity = '0.5'; apiBtn.disabled = true;

       let res = false;
       if (isLongTermMode) {
           const finalTitle = activeNote.title.trim() === "" ? "제목 없음" : activeNote.title.trim();
           res = await apiLongTermSave(finalTitle, pureText);
       } else {
       ...
       apiBtn.disabled = false; apiBtn.style.opacity = '1'; apiBtn.innerText = `${modeName} 업데이트`;

       if (res) {
        const toast = document.createElement('div'); toast.innerText = `현재 채팅방의 ${modeName}에 성공적으로 반영되었습니다.`;
        ...
        document.body.appendChild(toast); setTimeout(() => { toast.style.opacity = '0'; setTimeout(() => toast.remove(), 300); }, 2500);
           ...
           alert(`${modeName} 업데이트에 실패했습니다. (네트워크 혹은 글자수 제한 초과)`);
       }
      };
     ...
     saveStatus.className = 'sn-save-status';
     saveStatus.style.cssText = "font-size: 12px; font-weight:500;";
 ...
 function injectToolbarButton() {
  ...
  tbBtn.onclick = (e) => {
   ...
   const popup = document.getElementById('simple-notepad-popup');
   if (popup) {
    if (popup.style.display === 'flex') popup.style.display = 'none';
    else { popup.style.display = 'flex'; isSidebarOpen = false; if (forceRenderUI) forceRenderUI(); }
   }
  };
 ...
 setInterval(() => { initNotepad(); injectToolbarButton(); }, 500);
