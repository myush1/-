...
(() => {
  ...
  const FolderManager = {
    getFolders() {
      try {
        const data = localStorage.getItem('crack_ui_font_folders');
        return data ? JSON.parse(data) : [];
      } catch { return []; }
    },
    saveFolders(folders) {
      localStorage.setItem('crack_ui_font_folders', JSON.stringify(folders));
    },
    async deleteFolder(folderName) {
      const folders = this.getFolders().filter(f => f !== folderName);
      this.saveFolders(folders);
      const allFonts = await FontDB.getAllFonts();
      for (const font of allFonts) {
        if (font.folder === folderName) {
          font.folder = '';
          await FontDB.saveFont(font);
        }
      }
    },
    renameFolder(oldName, newName) {
      ...
      if (folders.includes(newName)) return false;
  ...
  async function applyFontGroup() {
    ...
    const RANGES = {
      latin: 'U+0020-007F, U+00A0-024F, U+0250-02AF, U+1E00-1EFF',
    ...
    function createVirtualFont(fontName, newFamilyName, range) {
      ...
      return font.css.replace(/@font-face\s*{([^}]+)}/gi, (match, inner) => {
        let newInner = inner.replace(/font-family\s*:\s*[^;]+;/gi, `font-family: '${newFamilyName}';`);
        if (range) {
          newInner = newInner.replace(/unicode-range\s*:\s*[^;]+;/gi, '');
    ...
    if (roomConfig) {
      ...
      if (roomConfig.cyrillic) {
        finalCSS += createVirtualFont(roomConfig.cyrillic, 'CrackVirtual_Cyrillic', RANGES.cyrillic) + '\n';
        fontFamilyStack.push("'CrackVirtual_Cyrillic'");
      }
      if (roomConfig.kana) {
    ...
    if (fontFamilyStack.length > 0) {
      const familyValue = fontFamilyStack.join(', ') + ', sans-serif';
      finalCSS += `
  ...
  // --- 폰트 UI용 동적 스타일 주입 (버튼 줄바꿈 방지 및 디자인 최적화) ---
  function ensureFontModuleStyles() {
    ...
    style.textContent = `
      ...
      .crack-ui-font-item-tag { color: rgba(255, 255, 255, .72); }
      .crack-ui-font-item-memo { color: rgba(255, 255, 255, 0.45); } /* '메모:' 텍스트 없이 내용만 노출 */
      ...
      .crack-ui-tag-grid { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 12px; }
      .crack-ui-tag-label {
        display: inline-flex; align-items: center; gap: 4px; font-size: 11px; font-weight: 700;
        background: rgba(255, 255, 255, 0.05); color: rgba(255, 255, 255, 0.7);
        padding: 5px 10px; border-radius: 999px; cursor: pointer; border: 1px solid transparent;
        transition: all 130ms ease; user-select: none;
      }
      .crack-ui-tag-label:hover { background: rgba(255, 255, 255, 0.1); }
      .crack-ui-tag-label input { display: none; }
      .crack-ui-tag-label:has(input:checked) { background: rgba(254, 69, 50, 0.14); color: #FE4532; border-color: rgba(254, 69, 50, 0.46); }
      ...
      html[data-theme="light"] .crack-ui-tag-label { background: rgba(17, 24, 39, 0.05); color: rgba(17, 24, 39, 0.7); }
      html[data-theme="light"] .crack-ui-tag-label:hover { background: rgba(17, 24, 39, 0.1); }
  ...
  const DEFAULT_TAGS = ['한국어', '알파벳', '가나', '키릴 문자', '한자'];
  ...
  function openFolderManagerModal() {
    ...
    const renderFolderList = () => {
      ...
      if (folders.length === 0) return '<div class="crack-ui-row-desc" style="text-align:center; margin: 16px 0;">등록된 폴더가 없습니다.</div>';
      return folders.map(f => `
        <div class="crack-ui-choice-row" style="display:flex; justify-content:space-between; margin-bottom:4px; min-height: 32px; padding: 6px 10px;">
          <span style="font-size: 12px; font-weight:700;">${f}</span>
          <button type="button" class="crack-ui-font-text-btn danger" data-delete-folder="${f}">삭제</button>
        </div>
      `).join('');
    };
    ...
    overlay.innerHTML = `
      <div class="crack-ui-font-modal" onclick="event.stopPropagation()">
        <div class="crack-ui-font-modal-title">
          <span>폴더 관리</span>
          <button type="button" class="crack-ui-font-text-btn" id="folder-close-x-btn">×</button>
        </div>

        <div style="display: flex; gap: 6px; margin-bottom: 12px;">
          <input type="text" id="new-folder-input" class="crack-ui-font-input" style="margin:0;" placeholder="새 폴더 이름">
          <button type="button" id="add-folder-btn" class="crack-ui-btn crack-ui-btn-danger" style="margin:0; flex-shrink:0;">추가</button>
        </div>

        <div id="folder-list-container" style="max-height: 200px; overflow-y: auto; margin-bottom: 16px;">
          ${renderFolderList()}
        </div>

        <button type="button" id="folder-done-btn" class="crack-ui-btn crack-ui-btn-primary" style="width: 100%;">완료</button>
      </div>
    `;
    ...
    overlay.querySelector('#folder-close-x-btn').addEventListener('click', closeModal);
    overlay.querySelector('#folder-done-btn').addEventListener('click', closeModal);

    const bindDeleteEvents = () => {
      overlay.querySelectorAll('[data-delete-folder]').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          const folderName = e.target.getAttribute('data-delete-folder');
          if (confirm(`'${folderName}' 폴더를 삭제하시겠습니까?\n(폴더 안의 폰트들은 기본 목록으로 이동합니다)`)) {
            await FolderManager.deleteFolder(folderName);
            overlay.querySelector('#folder-list-container').innerHTML = renderFolderList();
            bindDeleteEvents();
          }
        });
      });
    ...
    overlay.querySelector('#add-folder-btn').addEventListener('click', () => {
      const input = overlay.querySelector('#new-folder-input');
      const val = input.value.trim();
      if (!val) return;
      const folders = FolderManager.getFolders();
      if (folders.includes(val)) { alert('이미 존재하는 폴더입니다.'); return; }
      folders.push(val);
  ...
  async function openFontSettingsModal(fontData) {
    ...
    allFonts.forEach(f => {
      (f.tags || []).forEach(t => { if (!DEFAULT_TAGS.includes(t)) customTags.add(t); });
    });

    const currentTags = fontData.tags || [];
    const currentFolder = fontData.folder || '';
    ...
    const renderTags = () => {
      ...
      DEFAULT_TAGS.forEach(tag => {
        ...
        html += `<label class="crack-ui-tag-label default-tag"><input type="checkbox" value="${tag}" ${isChecked}> ${tag}</label>`;
      });
      customTags.forEach(tag => {
        const isChecked = currentTags.includes(tag) ? 'checked' : '';
        html += `<label class="crack-ui-tag-label"><input type="checkbox" value="${tag}" ${isChecked}> ${tag}</label>`;
      });
      return html;
    };

    const renderFolderOptions = () => {
      let html = `<option value="">지정 안 함</option>`;
      allFolders.forEach(f => {
        const isSel = (f === currentFolder) ? 'selected' : '';
        html += `<option value="${f}" ${isSel}>${f}</option>`;
      });
      return html;
    };
    ...
    overlay.innerHTML = `
      <div class="crack-ui-font-modal" onclick="event.stopPropagation()">
        ...
        <label class="crack-ui-font-modal-label">소속 폴더</label>
        <select id="modal-folder" class="crack-ui-font-select">
          ${renderFolderOptions()}
        </select>
        ...
        <label class="crack-ui-font-modal-label">태그 설정</label>
        <div class="crack-ui-tag-grid" id="modal-tag-grid">${renderTags()}</div>

        <div style="display: flex; gap: 6px; margin-bottom: 16px;">
          <input type="text" id="modal-new-tag" class="crack-ui-font-input" style="margin:0;" placeholder="새 태그 (예: 감성, 고딕)">
          <button type="button" id="modal-add-tag-btn" class="crack-ui-btn crack-ui-btn-danger" style="margin:0; flex-shrink:0;">추가</button>
        </div>

        <div class="crack-ui-btn-group">
    ...
    overlay.querySelector('#modal-add-tag-btn').addEventListener('click', () => {
      const newTagInput = overlay.querySelector('#modal-new-tag');
      const newTag = newTagInput.value.trim();
      if (newTag && !DEFAULT_TAGS.includes(newTag) && !customTags.has(newTag)) {
        customTags.add(newTag);
        currentTags.push(newTag);
        overlay.querySelector('#modal-tag-grid').innerHTML = renderTags();
        newTagInput.value = '';
      }
    });

    overlay.querySelector('#modal-save-btn').addEventListener('click', async () => {
      const checkedBoxes = overlay.querySelectorAll('#modal-tag-grid input:checked');
      const selectedTags = Array.from(checkedBoxes).map(cb => cb.value);

      const newFolder = overlay.querySelector('#modal-folder').value;
      const newMemo = overlay.querySelector('#modal-memo').value.trim();

      const updatedFont = { ...fontData, tags: selectedTags, folder: newFolder, memo: newMemo };
  ...
  async function syncFontUI() {
    ...
    const selCyrillic = document.getElementById('crack-ui-font-sel-cyrillic');
    ...
    const config = roomConfig || { roomId, body: '', code: '', title: '', latin: '', cyrillic: '', kana: '', hanzi: '' };
    ...
    const createFontElement = (f) => {
      ...
      let detailsHtml = '';
      if (f.tags && f.tags.length > 0) detailsHtml += `<span class="crack-ui-font-item-tag">태그: ${f.tags.join(', ')}</span>`;
      // '메모:' 텍스트 없이 내용만 노출
      ...
      item.querySelector('.crack-ui-font-del-btn').addEventListener('click', async (e) => {
        ...
        if(window.confirm(`'${f.name}' 폰트를 보관함에서 삭제하시겠습니까?`)) {
          try {
            ...
            if (currentConfig) {
              let updated = false;
              ['body', 'code', 'title', 'latin', 'cyrillic', 'kana', 'hanzi'].forEach(k => {
    ...
    if (allFonts.length === 0) {
      ...
      const managedFolders = FolderManager.getFolders();
      const folderGroups = {};
      managedFolders.forEach(folder => { folderGroups[folder] = []; });
      const rootFonts = [];

      allFonts.forEach(f => {
        if (f.folder && managedFolders.includes(f.folder)) {
          folderGroups[f.folder].push(f);
        } else {
          rootFonts.push(f);
        }
      });

      managedFolders.forEach(folderName => {
        const fontsInFolder = folderGroups[folderName];
        const section = document.createElement('div');
        section.className = 'crack-ui-section';
        section.dataset.open = '0';
        section.innerHTML = `
          <button type="button" class="crack-ui-section-head">
            <span><span class="crack-ui-section-title">${folderName} <span style="opacity:0.5; font-weight:normal; margin-left:4px;">(${fontsInFolder.length})</span></span></span>
            <span class="crack-ui-section-chevron" aria-hidden="true">▾</span>
          </button>
          <div class="crack-ui-section-body" hidden></div>
        `;

        const headBtn = section.querySelector('.crack-ui-section-head');
        const bodyDiv = section.querySelector('.crack-ui-section-body');
        ...
        if (fontsInFolder.length === 0) {
          bodyDiv.innerHTML = `<span class="crack-ui-row-desc" style="padding:0 8px;">비어있음</span>`;
        } else {
          fontsInFolder.forEach(f => bodyDiv.appendChild(createFontElement(f)));
        }
        listWrapper.appendChild(section);
      });

      if (rootFonts.length > 0) {
        rootFonts.forEach(f => listWrapper.appendChild(createFontElement(f)));
      }
    }

    // 필터링 및 즐겨찾기 상단 정렬
    const generateOptions = (targetTag) => {
      let filteredFonts = allFonts;
      // 타겟 태그가 있는 경우, 해당 태그를 선택한 폰트만 남김 (엄격한 필터링)
      if (targetTag) {
        filteredFonts = allFonts.filter(f => (f.tags || []).includes(targetTag));
      }

      const sorted = filteredFonts.sort((a, b) => {
        // 즐겨찾기 한 폰트가 무조건 위로 오도록 정렬
      ...
      return `<option value="">기본</option>` +
        sorted.map(f => {
          // 즐겨찾기는 깔끔한 텍스트 별모양(★)으로 표시
          const mark = f.favorite ? '★ ' : '';
          return `<option value="${f.name}">${mark}${f.name}</option>`;
        }).join('');
    };

    if(selBody) selBody.innerHTML = generateOptions('한국어');
    if(selCode) selCode.innerHTML = generateOptions(''); // 태그 없음 (모두 표시)
    if(selTitle) selTitle.innerHTML = generateOptions(''); // 태그 없음 (모두 표시)
    if(selLatin) selLatin.innerHTML = generateOptions('알파벳');
    if(selCyrillic) selCyrillic.innerHTML = generateOptions('키릴 문자');
    if(selKana) selKana.innerHTML = generateOptions('가나');
    if(selHanzi) selHanzi.innerHTML = generateOptions('한자');
    ...
    if(selCyrillic) selCyrillic.value = config.cyrillic || '';
  ...
  async function handleSelectChange() {
    const roomId = getCurrentRoomId();
    let roomConfig = (await FontDB.getRoomConfig(roomId)) || { roomId, body: '', code: '', title: '', latin: '', cyrillic: '', kana: '', hanzi: '' };
    ...
    roomConfig.cyrillic = document.getElementById('crack-ui-font-sel-cyrillic').value;
  ...
  function injectFontThemeUI() {
      ...
      <div class="crack-ui-section-body" data-crack-ui-section-body="font" ${savedFontOpen ? '' : 'hidden'}>
        <div class="crack-ui-choice-group">
          <div class="crack-ui-choice-head" style="margin-bottom: 8px;">
            ...
            <button type="button" class="crack-ui-font-text-btn" id="crack-ui-folder-manage-btn">폴더 관리</button>
          </div>

          <div id="crack-ui-font-list-wrapper" class="crack-ui-font-list-wrapper"></div>

          <textarea id="crack-ui-font-add-css" class="crack-ui-font-input crack-ui-font-textarea" style="min-height: 50px; font-size:11px;" placeholder="눈누 폰트 @font-face 코드 또는 구글 폰트 <link> 코드 입력"></textarea>
        ...
        <div class="crack-ui-choice-group">
          ...
          <div id="crack-ui-multi-font-panel" class="crack-ui-multi-font-panel" style="display: ${savedMultiFontOpen ? 'block' : 'none'};">
            <div style="margin-top: 0;">
              <div class="crack-ui-row-desc">영문 (알파벳 전체)</div>
              <select id="crack-ui-font-sel-latin" class="crack-ui-font-select"></select>
            </div>
            <div style="margin-top: 8px;">
              <div class="crack-ui-row-desc">러시아어 (키릴 문자)</div>
              <select id="crack-ui-font-sel-cyrillic" class="crack-ui-font-select"></select>
            </div>
            <div style="margin-top: 8px;">
    ...
    document.getElementById('crack-ui-folder-manage-btn').addEventListener('click', (e) => {
      e.preventDefault();
    ...
    addBtn.addEventListener('click', async (e) => {
      e.preventDefault();
      const inputText = addCssInput.value.trim();
      if (!inputText) return alert('눈누 @font-face 코드나 구글 폰트 <link>, @import 코드를 입력해주세요.');

      addBtn.textContent = '등록 중...';
      addBtn.disabled = true;

      try {
        let combinedCSS = inputText;
        const urlRegex = /(?:href|url)\s*[=(]\s*(['"]?)(https:\/\/fonts\.googleapis\.com\/css2\?[^'"\s>)]+)\1/gi;
        let match;
        const fetchPromises = [];
        ...
        while ((match = urlRegex.exec(inputText)) !== null) {
          ...
          fetchPromises.push(
            fetch(fontUrl)
              .then(res => res.text())
              .catch(err => {
                console.warn("[Crack UI Font] 구글 폰트 Fetch 실패:", fontUrl, err);
                return '';
              })
          );
        }
        ...
        for (const name of familyNames) {
          const fontData = {
            ...
            name: name,
            css: fontGroups[name].join('\n\n'),
            tags: [], folder: '', memo: '', favorite: false
          };
    ...
    const selects = ['body', 'code', 'title', 'latin', 'cyrillic', 'kana', 'hanzi'];
    selects.forEach(id => {
